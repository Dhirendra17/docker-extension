...
```shell
  docker buildx build -t docker/dhirendra777:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install docker/dhirendra777:latest
```
...
```shell
  docker extension dev ui-source docker/dhirendra777:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug docker/dhirendra777:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset docker/dhirendra777:latest
```
...
```shell
docker extension update docker/dhirendra777:latest
```
...
```shell
docker extension rm docker/dhirendra777:latest
```
